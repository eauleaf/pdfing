...
pdf_abridge <- function(pdf_path,
  ...
  if (file.exists(rds_path)) {
    txt <- read_rds(rds_path)
  } else {
    txt <- pdftools::pdf_text(pdf_path) %>% purrr::map(read_lines)
  ...
  for(pattern in patterns){
    ...
    if( nrow(search_results) > 0 ){
      ...
      search_results <- set_names(search_results, c('page_number','line_text'))
      remaining_pages <- search_results %>% pull(1) %>% as.numeric()
      if(remove_whitespace){ search_results <- mutate(search_results, line_text = stringr::str_squish(line_text)) }
  ...
  if(open_pdf){ open_this(save_path) }
